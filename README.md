...
- 审批失败保留卡片；过期/正在提交时禁止再次授权。服务端允许时显示「永久允许」，需二次确认；永久授权范围和撤销在服务端管理。
- 收到当前运行启动回执后可继续输入消息加入服务端队列，支持取消及断线恢复队列快照，不自动重发。
- AI 回复下方可点击「语音回复」，顶部扬声器开关可自动朗读前台新完成回复。需在 Web 配置 TTS，手机不保存供应商密钥；切换聊天或进入后台停止。
- 首次发送避免被旧活动快照提前 resume；真实失败仍保留错误信息。详见 docs/release-1.0.8.md。

### 1.0.10 文件与语音附件

大文件可流式下载保存；AI 返回的音频附件支持聊天内播放和停止，下载仍独立保留。点击 Markdown 本地音频链接直接播放，不再强制下载。现成音频播放不需要 TTS 配置，只有文字合成语音需要服务端 TTS。此前报告的聊天恢复重复问题仍在排查。

### 1.0.11 语音交互纠正（替代 1.0.8 自动朗读说明）

普通文字回复不再有语音按钮或自动朗读开关。只有 AI 实际返回的音频文件/链接显示播放与「转文字」。转文字需当前 Profile 配置 STT；用户主动点击后识别，结果显示在音频卡下方，不作为新聊天消息发送。

### 1.0.13 自动加载历史

查看较早聊天时，接近历史顶部会自动加载下一页，不再需要点击“加载更早”。失败时保留原位置并显示重试，离线/恢复中暂停，不重复发起分页请求。


### 当前工作区：前台恢复去重修复（待回归收口）

针对 Android App 从后台恢复、断网后重新联网并完成同步时，最新助手消息出现部分重复的问题，当前实现增加了以下保护：

- 记录当前连接期间已应用的流式事件，在恢复快照/事件回放时按事件身份抵扣，避免同一 `message.delta` 被再次拼接；
- 恢复时按 `run_id` 保留正在生成的助手气泡及本地前缀，并合并服务端补发的尾部内容；
- 事件回放使用独立 reducer，避免恢复过程再次污染实时事件去重状态；
- 新增网络重连场景的回归测试，覆盖“前半段 + 中段已实时显示，恢复仅补发后半段”的情况。

当前代码已成功生成 Android Release APK，但完整回归仍发现 2 项失败测试（多会话后台审批时序、恢复快照中相同文本的历史轮次保留），因此本修复尚未标记为全部验证通过，提交前需继续修正并重新运行完整测试。

### 1.0.14 前台恢复

修复 App 从后台回到前台时，正在生成的最后一条 AI 消息偶发重复/闪动：恢复会按当前 run 的快照与事件重叠合并，并保留流式气泡身份，不会重发用户消息。

...
- 恢复时在独立 reducer 中按顺序重放服务端保留的事件，不再按内容抵扣已实时接收的事件，避免丢失审批、澄清和工具状态。
- 完整回放重建正文后再合并快照；截断回放仅与同一 run 的本地缓存进行重叠合并，正常重复词语不会被当成重复事件丢弃。
- 仅复用同一 run 的 pending assistant 气泡，不再将其他轮次的正文、思考或 renderKey 拼入当前回答。
- 修复原先两项失败回归，新增 13 项状态完整性测试。完整本地测试 189 项通过、8 项按环境跳过；详情与验证边界见 [1.0.15 发布记录](docs/release-1.0.15.md)。

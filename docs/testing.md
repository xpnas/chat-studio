...
## 当前验证（2026-09-17，1.0.23）

- 图片预览改为边到边全屏，右下角采用大号图标化下载/关闭控件，覆盖浅深主题、横竖屏安全区、缩放、失败重试与保存中关闭回归。
- Flutter 静态分析通过；完整 Flutter 测试套件 278 项通过、11 项按环境跳过；图片预览与相关 Agent 预览测试通过。
- Android 产物及校验见 [1.0.23 交付记录](release-1.0.23.md)。

## 当前验证（2026-09-16，1.0.22）

- 聊天外边距改为左右各 15，用户/AI/计划卡片对齐；更新布局断言，15.5 正文字号及气泡内边距不变。
- **270 项单元/Widget 测试通过，11 项环境跳过**；静态分析、格式和应用标识检查通过。
- 字体截图测试单独通过，重新渲染 **23 张 PNG**，已查看更新后的聊天布局。
- 本次只调整间距，没有修改服务协议；未重复运行 10 项真实服务契约（上次 1.0.21 全部通过），也未进行真机、iOS 编译或远端 Actions 验证。
- Android 产物及校验见 [1.0.22 交付记录](release-1.0.22.md)。

## 先前验证（2026-09-16，1.0.21）

- **270 项单元/Widget 测试通过，11 项环境跳过**（10 项真实协议与 1 项字体预览单独运行）。新增 6 项聊天边距/字号、选中语义、按压涟漪及模型列表上滑绘制回归。
- 浅深主题的像素回归曾在旧实现上复现失败；修复后，100 模型长列表多次滚动/返回时，标题与搜索框区域像素保持不变。
- 静态分析无问题、Dart 格式与应用命名门禁通过。
- 截图测试 1 项通过，重新生成 **23 张预览**，新增浅深主题长模型列表滚动状态。
- 10 项隔离真实 Studio HTTP/Socket.IO 契约全部通过；Android 包已构建及校验，见 [本次交付记录](release-1.0.21.md)。没有 Android 真机、iOS 本地编译或远端 GitHub Actions 执行。

## 先前验证（2026-09-16，1.0.20）

- **264 项单元/Widget 测试通过，11 项环境跳过**（10 项真实协议与 1 项字体预览另行运行），新增 15 项横滑、服务器工作区、异步隔离和统一字号回归。
- 静态分析无问题，Dart 格式与应用命名门禁通过。
- **10 项真实 Studio HTTP/Socket.IO 契约通过**，新增独立服务器目录的创建、绝对路径持久化与文件列表读取；其余断网恢复、计划、队列、附件和语音协议继续通过。
- 发现并修正上一轮模型列表的一处既有回归：当前提供商强制展开导致点击无法折叠。保留当前项置顶，仅取消强制重复展开条件，原有完整回归通过。
- 当前预览为 **21 张**，包括服务器工作区及目录选择；由 Flutter 组件渲染，不是真机截图。
- Android 产物按版本输出到 `dist/`，详见 [发布与签名记录](release-1.0.20.md)。没有本机 iOS 编译、真机端到端或远端 Actions 执行。

## 先前验证（2026-09-16，1.0.19）
